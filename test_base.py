...
local_model_path = "D:\model\Qwen3-TTS-12Hz-0.6B-Base"
...
# 6. 跳过参考音频下载（使用内置 speaker）
p("\n6. 跳过参考音频下载...")
p("   ✅ 直接使用内置 speaker 'vivien' 进行测试")
ref_audio_ok = True
...
try:
    ...
    # 使用模型的 generate 方法
    input_ids = model.processor(text=test_text, return_tensors="pt", padding=True)['input_ids'].to(model.device)
    
    # 对于 Base 模型，需要使用 generate 方法
    ...
    # 解码生成的代码
    wavs, sr = model.speech_tokenizer.decode([{"audio_codes": codes} for codes in talker_codes_list])
